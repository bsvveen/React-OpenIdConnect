...
class Authenticate extends Component {
    ...
    this.userManager.getUser().then((user) => {
      if (user !== null && user !== undefined) {
        ...
        this.userManager
          ...
          .then(() => {
            window.history.replaceState({}, "", this.props.redirect_uri);
          })
          .catch(function (err) {
  ...
  render() {
    if (this.state.isAuthenticated) {
      ...
      return <div>{this.props.renderLoading()}</div>;
    }
    return (
      <div>{this.props.renderNotAuthenticated({ onSignIn: this.signin })}</div>
    );
  }
}
